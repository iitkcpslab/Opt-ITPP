...
protected:
	...
	// check whether two nodes could be mutexed return true if they are mutexed
	bool should_be_fwd_mutexed(MDDNode *node_a, MDDNode *node_b);

	bool should_be_fwd_mutexed(MDDNode *node_a, MDDNode *node_a_to, MDDNode *node_b, MDDNode *node_b_to);

	bool should_be_bwd_mutexed(MDDNode *node_a, MDDNode *node_b);
	bool should_be_bwd_mutexed(MDDNode *node_a, MDDNode *node_a_to, MDDNode *node_b, MDDNode *node_b_to);

	void add_bwd_node_mutex(MDDNode *node_a, MDDNode *node_b);

	void add_fwd_node_mutex(MDDNode *node_a, MDDNode *node_b);
	void add_fwd_edge_mutex(MDDNode *node_a, MDDNode *node_a_to, MDDNode *node_b, MDDNode *node_b_to);
...
public:
	ConstraintPropagation(MDD *mdd0, MDD *mdd1) : mdd0(mdd0), mdd1(mdd1) {}
