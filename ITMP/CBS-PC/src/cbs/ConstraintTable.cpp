...
void ConstraintTable::buildCAT(int agent, const vector<Path *> &paths, size_t _cat_size)
{
	...
	if (map_size < map_size_threshold)
	{
		...
		for (size_t ag = 0; (int)ag < (int)paths.size(); ag++)
		{
			if ((int)ag == agent || paths[ag] == nullptr || paths[ag]->size() == 0)
				continue;
			for (size_t timestep = 0; (int)timestep < (int)paths[ag]->size(); timestep++)
			{
			...
			for (size_t timestep = paths[ag]->size(); (int)timestep < cat_size; timestep++)
	...
	else
	{
		...
		for (size_t ag = 0; (int)ag < (int)paths.size(); ag++)
		{
			...
			int curr;
			for (size_t timestep = 1; (int)timestep < (int)paths[ag]->size(); timestep++)
			{
			...
			int goal = paths[ag]->back().location;
			for (size_t timestep = paths[ag]->size(); (int)timestep < (int)cat_size; timestep++)
