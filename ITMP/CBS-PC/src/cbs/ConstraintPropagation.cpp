...
void ConstraintPropagation::init_mutex()
{
	int num_level = std::min(mdd0->flatten_levels.size(), mdd1->flatten_levels.size());
	// node mutex
	for (int i = 0; i < num_level; i++)
	{
		// COMMENT unordered map can be changed to vector for efficiency
		auto loc2mdd = collectMDDlevel(mdd0, i);
		for (MDDNode *it_1 : mdd1->flatten_levels[i])
		{
	...
	for (int i = 0; i < num_level - 1; i++)
	{
		...
		for (auto &node_0 : mdd0->flatten_levels[i])
		{
...
void ConstraintPropagation::fwd_mutex_prop()
{
	...
	std::vector<boost::unordered_set<edge_pair>> to_check(max(mdd0->flatten_levels.size(), mdd1->flatten_levels.size()));
...
{
	// level_0 < mdd_s->flatten_levels, the index of the level
	MDD *mdd_s = mdd0;
	MDD *mdd_l = mdd1;
	if (level_0 > level_1)
	{
	...
	if (level_0 > (int)mdd_s->flatten_levels.size())
	{
		std::cout << "ERROR!" << std::endl;
	}
	if (level_1 > (int)mdd_l->flatten_levels.size())
	{
		std::cout << "ERROR!" << std::endl;
	}
	...
	for (auto &it : mdd_l->flatten_levels[level_0])
	{
...
std::pair<std::vector<Constraint>, std::vector<Constraint>> ConstraintPropagation::generate_constraints(int level_0, int level_1)
{
	...
	for (int lvl = 0; lvl <= level_0; lvl++)
	{
		...
		for (auto &it : mdd_s->flatten_levels[lvl])
		{
			if (it->cost <= level_0)
			{
		...
		for (auto &it : mdd_l->flatten_levels[lvl])
		{
			if (it->cost <= level_1)
			{
